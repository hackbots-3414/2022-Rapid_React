...
public class RobotContainer {
    ...
    private RobotContainer() {
        ...
        m_chooser.setDefaultOption("Any Tarmac - 0 Ball", new ZeroBall(m_drivetrain));
        ...
        m_chooser.addOption("Tarmac 1 - 3 Ball", new ThreeBall(m_drivetrain, m_belt, m_shooter));
    ...
    private void configureButtonBindings() {
        ...
        shootHighFarButton.whileTrue(new ShootCommand(m_belt, m_shooter, 3, Constants.ShooterConstants.shooterTimer));
        shootButton.whileTrue(new RunShoot(m_belt, m_shooter, true, Constants.ShooterConstants.shooterTimer));
        ejectButton.whileTrue(new Eject(m_belt));
        //beltButton.whileTrue(new RunBelt(m_belt), true);
        intakeButton.whileTrue(new BeltCommand(m_belt));
        shootHighButton.whileTrue(new ShootCommand(m_belt, m_shooter, 1, Constants.ShooterConstants.shooterTimer));
        shootLowButton.whileTrue(new ShootCommand(m_belt, m_shooter, 2, Constants.ShooterConstants.shooterTimer));
        climberUpButton.onTrue(new ClimberUpCommand(m_climber));
        climberDownButton.onTrue(new ClimberDownCommand(m_climber));
        // eatBallButton.whileTrue(new EatBall(m_drivetrain, m_pixy), true);
        //beltButton.whileTrue(new BeltCommand(m_belt));
        pinRetract.onTrue(new PinRetractCommand(m_climber));
        pinRetract2.onTrue(new PinRetractCommand(m_climber));
        revShooter.whileTrue(new RevShooterCommand(m_shooter));
